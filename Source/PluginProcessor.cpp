...
{
	NormalisableRange<float> delayTimeRange(0.0f, 25.0f, 0.5f);
	NormalisableRange<float> offSetRange(0.0f, 15.0f, 0.5f);
...
void MajorProjectAudioProcessor::updateFilter()
{
	...
	*highShelfFilter.state = *dsp::IIR::Coefficients<float>::makeHighShelf(mSampleRate, 1000.0f, 0.5f, 0.35f);
}
...
void MajorProjectAudioProcessor::processBlock(AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
	...
	dsp::AudioBlock<float> block(mDoublerBuffer);
	dsp::AudioBlock<float> block2(mDoublerBuffer2);
	updateFilter();
	//highShelfFilter.process(dsp::ProcessContextReplacing<float>(block.getSingleChannelBlock(0)));
	//highShelfFilter.process(dsp::ProcessContextReplacing<float>(block.getSingleChannelBlock(0)));
	...
	for (int channel = 0; channel < totalNumInputChannels; ++channel)
	{
		...
		delayTime(doublerBufferLength, buffer, bufferLength, doublerBufferData, doublerBufferData2);
	}
...
void MajorProjectAudioProcessor::delayTime(const int doublerBufferLength, AudioBuffer<float>& buffer, const int bufferLength, const float* doublerBufferData, const float* doublerBufferData2)
{
